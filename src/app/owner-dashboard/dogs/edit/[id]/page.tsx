...
export default function EditDogPage() {
  ...
  const { user } = useAuth();
  const { getDogById, updateDog } = useData();
  ...
  useEffect(() => {
    ...
    const loadDog = async () => {
      try {
        ...
        // First try to get dog from context
        let dogData = getDogById(dogId);
        
        // If not found in context, fetch directly from API
        if (!dogData) {
          console.log("Dog not found in context, fetching from API");
        ...
        if (dogData && dogData.ownerId === user.profileId) {
          ...
          setError("This dog doesn't belong to your account or doesn't exist");
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const updatedDog = await updateDog(dogId, dogData);
      console.log('Dog updated successfully:', updatedDog);
      
      // Redirect back to the dog details page
