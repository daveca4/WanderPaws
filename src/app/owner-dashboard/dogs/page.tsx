...
export default function DogsPage() {
  ...
  const { data: dogs, isLoading, error } = useOwnerDogs(user?.profileId);
  ...
  if (error) {
    return (
      <div className="bg-white shadow rounded-lg p-6 text-center">
        ...
        <p className="text-gray-500 mb-4">{error instanceof Error ? error.message : "Failed to load dogs"}</p>
      </div>
    );
  }
